...
// Chart setup
function dailyRevenue(element, data, icon) {
    var height = 50;
    var dataset = data
    ...
    var tooltip = d3.tip()
        ...
        .html(function (d) {
            return "<ul class='list-unstyled mb-5'>" +
                "<li>" + "<div class='text-size-base mt-5 mb-5'>" + icon + d.date2 + "</div>" + "</li>" +
                "<li>" + "<span class='text-semibold pull-right'>" + formatMoney(d.alpha) + " ₺</span>" + "</li>" +
                "</ul>";
        });
...
// Chart setup
function generateBarChart(element, dataKey, dataValue, color) {
    ...
    tip.html(function (d, i) {
        var monthName = dataValue[i];
        var cost = dataKey[i];
        return "<div class='text-center'>" +
            "<h6 class='no-margin'>" + monthName + "</h6>" +
            "<span class='text-size-small'>" + formatMoney(cost, 2) + " ₺</span>" +
            "</div>";
    });
...
// Chart setup
function progressCounter(element, radius, border, color, end, iconClass, textTitle, textSubTitle, textAverage,percentageText) {
    ...
    var numberText = d3.select(element)
        .append('h2')
        .attr('class', 'mt-15 mb-5 cursor-pointer')
        .attr('title', percentageText)
    ...
    d3.select(element)
        .append('div')
        .attr('class', 'text-size-small text-muted text-orange text-italic')
        .text(textSubTitle);
        ...
        d3.bullet = function () {
            ...
            function bullet(g) {
                g.each(function (d, i) {
                    ...
                    // Retrieve the old x-scale, if this is an update.mt-15 mb-5
