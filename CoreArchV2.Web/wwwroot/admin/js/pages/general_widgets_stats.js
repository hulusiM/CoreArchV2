...
// Chart setup
function animatedPieWithLegend(element, data, formatType) {
    var extension = "";
    ...
    // Create chartmt-15 mb-5 text-semibold
    // ------------------------------
    ...
    // Colors
    var colors = ['#FFB74D', '#4FC3F7', '#81C784', '#FF0000', '#00FF00'];
    ...
    d3Container.select('h2')
        ...
        .tween("text", function (d) {
            ...
            return function (t) {
                var text = "";
                if (formatType == 'money') {
                    text = formatMoney(sum,0) + extension;
                } else {
                    text = d3.format(",d")(Math.round(i(t))) + extension;
                }
    ...
    // Add elementmt-15 mb-5 text-semibold
    ...
    if (formatType == 'money') {
        legend.append('span')
            .text(function (d, i) {
                return formatMoney(d.data.value,0) + extension;
            });
    } else {
        legend.append('span')
            .text(function (d, i) {
                return d.data.value + extension;
            });
    }
}
...
// Chart setup
function pieArcWithLegend(element, data) {
    var size = 250;
    // Main variables
    var d3Container = d3.select(element),
        distance = 2, // reserve 2px space for mouseover arc moving
    ...
    var tip = d3.tip()
        ...
        .html(function (d) {
            return "<ul class='list-unstyled mb-5'>" +
                "<li>" + "<div class='text-size-base mb-5 mt-5'>" + d.data.status + "</div>" + "</li>" +
                "<li>" + "Toplam: &nbsp;" + "<span class='text-semibold pull-right'>" + d.value + "</span>" + "</li>" +
                "<li>" + "Share: &nbsp;" + "<span class='text-semibold pull-right'>" + (100 / (sum / d.value)).toFixed(2) + "%" + "</span>" + "</li>" +
                "</ul>";
        });
    ...
    // Colors
    var colors = ['#78909C', '#F06292', '#4DB6AC', '#FF0000', '#00FF00'];
...
// Chart setup
function donutWithDetails(element, data, description = "") {
    var size = 146;
    // Main variables
    var d3Container = d3.select(element),
        distance = 2, // reserve 2px space for mouseover arc moving
    ...
    // Colors
    var colors = ['#2196F3', '#F44336', '#4CAF50', '#FF5722', '#00BCD4', '#E91E63', '#9C27B0',
        '#673AB7', '#3F51B5', '#03A9F4', '#009688', '#8BC34A', '#FF9800', '#795548', '#777777',
        '#607D8B', '#37474F', '#1565C0', '#90A4AE', '#283593', '#4DB6AC', '#00FF00', '#7986CB'];
    // Append path
    var arcPath = arcGroup
        .append("path")
        .style("fill", function (d, i) {
